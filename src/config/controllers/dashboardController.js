...
export const getDashboardStat = async (req, res) => {
    const [totalBooks, totalReviews, totalMembers, recentBooks, allBooksWithRatings] = await Promise.all([
        prisma.books.count(),
        ...
        prisma.books.findMany({
            ...
            include: {
                reviews: { select: { star: true } }, // ✅ Fix: select goes INSIDE the relation, not inside include directly
            },
        }),
        ...
        prisma.books.findMany({
            include: {
                reviews: { select: { star: true } }, // ✅ Same fix here
            },
        }),
    ]);
